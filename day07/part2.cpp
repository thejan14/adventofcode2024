...
bool checkOperators(ull const& testValue, std::vector<ull> const& numbers, ull acc, int index)
{
    ...
    else if (checkOperators(testValue, numbers, acc * numbers[index], index + 1))
    {
        return true;
    }
    else
    {
        auto combiend = stoull(std::to_string(acc) + std::to_string(numbers[index]));
        return checkOperators(testValue, numbers, combiend, index + 1);
    }
}
