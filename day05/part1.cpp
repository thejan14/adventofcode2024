...
int checkPrint(std::map<int, std::vector<int>>& rules, std::vector<int> const& print)
{
    for (auto pageIt = print.begin(); pageIt < print.end(); ++pageIt)
    {
        auto const pagePos = pageIt - print.begin();
        if (auto const ruleIt = rules.find(*pageIt); ruleIt != rules.end())
        {
...
int main()
{
    ...
    auto answer = 0;
    for (auto const& print : prints)
    {
        if (checkPrint(rules, print))
        {
